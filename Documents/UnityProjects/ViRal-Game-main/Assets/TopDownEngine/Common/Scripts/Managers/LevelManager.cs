...
{	
	...
	{	
		...
		{
			if(GameManager.Instance.UnlockedLevel < SceneManager.GetActiveScene().buildIndex)
            {
				GameManager.Instance.UnlockedLevel = SceneManager.GetActiveScene().buildIndex;
		...
		public void StorePointToGameManager()
        {
			GameManager.Instance.AddPoints(Point);
			int score = PlayerPrefs.GetInt("Highscore", 0);
			if(score < GameManager.Instance.Points){
