...
public class MainMenuManager : MonoBehaviour
{
    ...
    [SerializeField] private TMP_Text loadSlot1Label;
    ...
    public void OnPlay()
    {
        ...
        LogButtonClick("Play");
        if (GameSession.Instance == null || !GameSession.Instance.StartNewGame())
        {
            DebugLog("New Game dibatalkan karena save lama gagal dibersihkan.", DebugLevel.Error);
            return;
        }
    ...
    public void OnOpenLoad()
    {
        ...
        RefreshLoadSlots();
        ShowOnly(loadPanel);
        SelectFirst(loadSlot1Button != null && loadSlot1Button.interactable
            ? firstOnLoad
            : loadBackButton);
    }
    ...
    public void LoadSlot(int slot)
    {
        if (slot != 1)
        {
            DebugLog("Prototype saat ini hanya memakai save slot 1.", DebugLevel.Warning);
            return;
        }

        if (GameSession.Instance == null || !GameSession.Instance.ContinueGame())
            DebugLog("Save belum ada, rusak, atau scene tujuan tidak valid.", DebugLevel.Warning);
    }

    public void DeleteSave()
    {
        if (GameSession.Instance != null && GameSession.Instance.DeleteSave())
        {
            DebugLog("Save slot 1 dihapus.", DebugLevel.Success);
            RefreshLoadSlots();
        }
        else
            DebugLog("Save slot 1 gagal dihapus.", DebugLevel.Error);
    }

    private void RefreshLoadSlots()
    {
        bool hasSave = SaveSystem.TryGetSaveTime(out System.DateTime savedAt);
        bool english = ControlSettings.Language == UiLanguage.English;

        if (loadSlot1Button != null) loadSlot1Button.interactable = hasSave;
        if (loadSlot1Label != null)
            loadSlot1Label.text = hasSave
                ? $"SLOT 1\n{savedAt:dd/MM/yyyy HH:mm}"
                : $"SLOT 1\n{(english ? "EMPTY" : "KOSONG")}";
        if (loadSlot2Button != null) loadSlot2Button.interactable = false;
        if (loadSlot2Label != null)
            loadSlot2Label.text = $"SLOT 2\n{(english ? "EMPTY" : "KOSONG")}";
    }
    ...
    private void GoToCharacterSelectOrScene()
    {
        ...
        else
        {
            ...
            if (!string.IsNullOrEmpty(fallbackScene))
            {
                ...
                SceneFlow.LoadScene(fallbackScene);
            }
            else
            {
