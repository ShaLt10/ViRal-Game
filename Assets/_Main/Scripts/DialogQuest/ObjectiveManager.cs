...
public class ObjectiveManager : MonoBehaviour
{
    ...
    public void RestoreFromSession()
    {
        ...
        foreach (var objective in allObjectives)
        {
            ...
            if (GameSession.Instance.IsObjectiveCompleted(objective.objectiveID))
            {
        ...
        Objective active = GetObjective(GameSession.Instance.Data.activeObjectiveId);
        if (active != null && active.status != ObjectiveStatus.Completed)
        {
            active.status = ObjectiveStatus.Active;
            active.currentCount = Mathf.Clamp(
                GameSession.Instance.Data.activeObjectiveCount, 0, active.requiredCount);
    ...
    public void StartObjective(string objectiveID)
    {
        ...
        ObjectiveStatus previousStatus = objective.status;
        int previousCount = objective.currentCount;
        Objective previousCurrent = currentObjective;
        bool previousActive = isObjectiveActive;
        ...
        if (GameSession.Instance == null
            || !GameSession.Instance.SetActiveObjective(objectiveID, objective.currentCount))
        {
            objective.status = previousStatus;
            objective.currentCount = previousCount;
            currentObjective = previousCurrent;
            isObjectiveActive = previousActive;
            Debug.LogError($"[ObjectiveManager] Objective '{objectiveID}' gagal dimulai karena save gagal.");
            return;
        }
    ...
    public void UpdateProgress(string objectiveID, int amount = 1)
    {
        ...
        else
        {
            ...
            {
                objective.currentCount = previousCount;
                Debug.LogError($"[ObjectiveManager] Progress '{objectiveID}' dibatalkan karena save gagal.");
                return;
            }
    ...
    public void CompleteObjective(string objectiveID)
    {
        ...
        if (GameSession.Instance == null || !GameSession.Instance.CompleteObjective(objectiveID))
        {
            objective.status = previousStatus;
            objective.currentCount = previousCount;
            currentObjective = previousCurrent;
            isObjectiveActive = previousActive;
            Debug.LogError($"[ObjectiveManager] Objective '{objectiveID}' dibatalkan karena save gagal.");
            return;
        }
    ...
    public void FailObjective(string objectiveID)
    {
        ...
        if (currentObjective == objective
            && (GameSession.Instance == null
                || !GameSession.Instance.SetActiveObjective("", 0)))
        {
            Debug.LogError($"[ObjectiveManager] Gagal menyimpan status failure '{objectiveID}'.");
            return;
        }
        ...
        if (obj != null)
        {
            if (currentObjective == obj
                && (GameSession.Instance == null
                    || !GameSession.Instance.SetActiveObjective("", 0)))
            {
                Debug.LogError($"[ObjectiveManager] Reset '{objectiveID}' dibatalkan karena save gagal.");
                return;
            }
    ...
    {
        ResetAllObjectives(true);
    }

    public void ResetAllObjectives(bool updateSave)
    {
        if (updateSave
            && (GameSession.Instance == null || !GameSession.Instance.ClearObjectives()))
        {
            Debug.LogError("[ObjectiveManager] Reset objective dibatalkan karena save gagal.");
            return;
        }
