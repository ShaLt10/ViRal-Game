...
public class SpotTheDifferenceSingleton : MonoBehaviour
{
    ...
    public void OnStageCleared()
    {
        ...
        {
            // Final clear
            bool progressSaved = GameSession.Instance != null
                && GameSession.Instance.CompleteMinigame("SpotTheDifference");
            SetAllGroups(false);
            if (winPanel) winPanel.SetActive(true);

            if (!progressSaved)
            {
                Debug.LogError("[SpotTheDifference] Progress gagal disimpan; perpindahan scene dibatalkan.");
            }
            else if (!string.IsNullOrEmpty(nextSceneOnFinish))
            {
                SceneFlow.LoadScene(nextSceneOnFinish);
            }
        }
    }
